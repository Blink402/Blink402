...
export default function GalleryViewPage() {
  ...
  const isOwner = connected && publicKey?.toBase58() === creatorWallet
  ...
  const checkAccess = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/gallery/${creatorWallet}/check-access?viewer=${publicKey.toBase58()}`
      )
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        {!isOwner && daysRemaining !== null && (
          <p className="text-neon-green-light text-sm font-mono">
