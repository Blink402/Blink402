...
export default function GallerySettingsPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const existingGalleryBlink = blinks.find(
          (b: any) =>
            b.endpoint_url?.includes(`/gallery/${publicKey.toBase58()}`) ||
            b.slug === `gallery-${publicKey.toBase58().slice(0, 8)}`
        )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/gallery/${publicKey.toBase58()}/configure`
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          price_usdc: price,
          payment_token: 'SOL', // Gallery always uses SOL
          access_duration_days: parseInt(duration),
          walletAddress: publicKey.toBase58() // For basic validation
        }),
      })
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <div className="text-neon-green-light text-2xl animate-pulse">
          Loading settings...
        </div>
      </div>
    )
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      ...
      {message && (
        <div className={`mb-6 p-4 border-2 ${
          message.type === 'success'
            ? 'border-neon-green-light bg-neon-green-light/10 text-neon-green-light'
      ...
      <div className="space-y-6 border-2 border-dashed border-neon-grey p-6 bg-neon-dark">
        {/* Note: Gallery always uses SOL for payments */}
        <div className="p-3 bg-neon-green-light/10 border border-neon-green-light/30 text-neon-green-light text-sm font-['Geist_Mono']">
          💡 Gallery access uses SOL payments (native Solana token)
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full p-3 bg-neon-black border-2 border-neon-grey text-neon-white font-['Geist_Mono'] focus:border-neon-green-light focus:outline-none"
            placeholder="0.05"
          />
        ...
        <div>
          ...
          <div className="p-3 bg-neon-black border-2 border-neon-grey font-['Geist_Mono'] text-sm text-neon-grey break-all">
            https://blink402.dev/gallery/{publicKey?.toBase58()}
          </div>
          {galleryBlink && (
            <a
              href={`/gallery/${publicKey?.toBase58()}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block mt-2 text-neon-green-light hover:underline text-sm"
            >
      ...
      {galleryBlink && (
        <div className="mt-4 p-6 bg-neon-dark border-2 border-dashed border-neon-green-light/30">
          <h3 className="text-lg font-['Geist_Sans'] mb-3 text-neon-green-light">
            ✅ Your Gallery is Ready!
          </h3>
          <p className="text-sm text-neon-grey font-['Geist_Mono'] mb-2">
            Share your gallery link with potential buyers:
          </p>
          <div className="p-3 bg-neon-black border border-neon-green-light/50 text-neon-green-light font-['Geist_Mono'] text-sm break-all">
            https://blink402.dev/gallery/{publicKey?.toBase58()}
          </div>
        </div>
      )}
