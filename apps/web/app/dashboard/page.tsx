...
export default function DashboardPage() {
  ...
  const { isAuthenticated, isAuthenticating, signIn, authToken, wallet } = useAuth()
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && authToken && wallet) {
      setIsLoading(true)
      getDashboardData(wallet, authToken)
        .then((dashboardData) => {
  ...
  }, [isAuthenticated, authToken, wallet])
  ...
  const handleToggleStatus = async (slug: string, currentStatus: string) => {
    ...
    try {
      ...
      await updateBlink(slug, { status: newStatus }, authToken)

      // Refresh dashboard data
      if (wallet) {
        const dashboardData = await getDashboardData(wallet, authToken)
        setData(dashboardData)
      }
    } catch (error) {
  ...
  // Show loading state
  if (isLoading || !data || isAuthenticating) {
    return (
      <div className="min-h-screen bg-neon-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-neon-grey font-mono text-sm">
            {isAuthenticating ? 'Authenticating...' : 'Loading dashboard...'}
          </p>
        </div>
      </div>
    )
      ...
      <section className="px-6 py-8">
        <div className="max-w-7xl mx-auto">
          ...
          <header className="flex items-center justify-between mb-12">
            ...
            <Link href="/create">
              <Button className="btn-primary">+ Create New Blink</Button>
            </Link>
          </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            <Card className="bg-neon-dark border-neon-blue-dark/20 p-6">
              ...
              <div className="text-neon-white font-mono text-3xl mb-1">${data.totalEarnings}</div>
            ...
            <Card className="bg-neon-dark border-neon-blue-dark/20 p-6">
              ...
              <div className="text-neon-white font-mono text-3xl mb-1">{data.totalRuns.toLocaleString()}</div>
            ...
            <Card className="bg-neon-dark border-neon-blue-dark/20 p-6">
              ...
              <div className="text-neon-white font-mono text-3xl mb-1">{data.activeBlinks}</div>
            ...
            <Card className="bg-neon-dark border-neon-blue-dark/20 p-6">
              ...
              <div className="text-neon-white font-mono text-3xl mb-1">${data.avgPrice}</div>
            ...
            <Card className="bg-neon-dark border-neon-blue-dark/20 overflow-hidden">
              ...
              {filteredBlinks.length === 0 && (
                <div className="p-12 text-center">
                  ...
                  {data.blinks.length === 0 ? (
                    <Link href="/create">
                      <Button className="btn-primary">Create Your First Blink</Button>
                    </Link>
                  ) : (
                    <Button
                      onClick={() => {
