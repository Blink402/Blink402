...
export default function GalleryManagePage() {
  ...
  const fetchImages = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/gallery/${publicKey.toBase58()}/images`
      )
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/gallery/${publicKey.toBase58()}/images/${id}`,
        {
          method: 'DELETE',
          headers: {
            'x-request-wallet': publicKey.toBase58()
          }
        }
      )
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <div className="flex gap-4 mt-2">
          <a
            ...
            className="text-neon-green-light hover:underline text-sm font-mono inline-block"
          >
            View Public Gallery →
          </a>
          <a
      ...
      {isLoading ? (
        <div className="text-center py-16">
          <div className="text-neon-green-light text-2xl font-mono animate-pulse">
            Loading gallery...
          </div>
        </div>
      ) : (
