...
export function DemoRunner() {
  ...
  return (
    <div className="space-y-6">
      {/* Status Bar */}
      <div className="flex items-center justify-between rounded-lg border border-dashed border-neon-green-light/30 bg-neon-dark px-6 py-4">
        <div className="flex items-center gap-3">
          <div
            className={`h-3 w-3 rounded-full ${
              state === 'success'
                ? 'bg-neon-green-light animate-pulse'
                : state === 'failed'
                ? 'bg-red-400'
                : isLoading
        ...
        <button
          ...
          className={`rounded-lg border-2 border-dashed px-6 py-2 font-medium transition-all ${
            isLoading
              ...
              : 'border-neon-green-light text-neon-green-light hover:border-neon-green-dark hover:bg-neon-green-light/10 hover:text-neon-green-dark hover:shadow-[0_0_20px_rgba(39,218,180,0.3)]'
          }`}
        >
          {isLoading ? (
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        {[
          { key: 'requesting', label: 'Transaction', icon: '📝' },
          { key: 'verifying', label: 'Verification', icon: '✓' },
          { key: 'calling', label: 'API Call', icon: '🔄' },
          { key: 'success', label: 'Result', icon: '🎉' },
        ].map((step, index) => {
            ...
            <div
              ...
              className={`rounded-lg border border-dashed p-4 text-center transition-all ${
                isActive
                  ...
                  : isCompleted
                  ? 'border-neon-green-light bg-neon-green-light/10'
                  : isFailed
      ...
      {/* Result */}
      {result && (
        <div className="rounded-lg border border-dashed border-neon-green-light bg-neon-green-light/5 p-6">
          <h3 className="mb-4 font-heading text-xl font-light text-neon-green-light">
            Dog Fact Result
          </h3>
