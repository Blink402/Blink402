...
export function DemoConsole({ logs, className = '' }: DemoConsoleProps) {
  ...
  return (
    <div className={`rounded-lg border border-neon-green-light/30 bg-neon-black ${className}`}>
      {/* Console Header */}
      <div className="border-b border-neon-green-light/30 px-4 py-2">
      ...
      {/* Console Footer */}
      <div className="border-t border-neon-green-light/30 px-4 py-2">
        <div className="flex items-center justify-between text-xs text-neon-grey">
          ...
          <span className="font-mono">
            {logs.length > 0 && logs[logs.length - 1].type === 'success' && (
              <span className="text-neon-green-light">● Ready</span>
            )}
